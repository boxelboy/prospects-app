      ...
      if (event.data === "send") {
          _.each(Prospects.find().fetch(), _.bind(function (prospect) {
              ...
              $.ajax({
                ...
                url: 'http://' + settings.server + ':' + port + '/api/BusinessMan/prospects',
                dataType: 'json',
                data: prospectObj,
                success: function(data) {
                  ...
                  $.ajax({
                    type: 'POST',
                    contentType: 'application/json',
                    url: 'http://' + settings.server + ':' + port +'/api/BusinessMan/notes',
                    dataType: 'json',
                    data: noteObj,
                    success: function() {
...
Template.body.events({
  ...
  'submit .login'(event) {
    ...
    $.ajax({
      type: 'GET',
      contentType: 'application/json',
      url: 'http://' + svr + ':' + port +'/api/BusinessMan/users?username=' + usr + '&password=' + pass,
      dataType: 'json',
      success: function(data) {
        if (data.total_items > 0) {
          Settings.update({_id: "1"},
            {$set: {user: usr, pwd: pass, server: svr}}
          );
          $("header").css("background-color","#438eb9");
          $('#spinner').hide();
        } else {
      ...
      error: function (jqXHR, textStatus, errorThrown) {
          $('#spinner').hide();
          $('#message').html('Unable to connect to server');
    ...
    if (target.button.value === 'save') {
      ...
      $.ajax({
        ...
        success: function(data) {
          var noteObj = JSON.stringify({
              ...
              $.ajax({
                ...
                success: function() {
                  $("header").css("background-color","#438eb9");
                  alert("Prospect saved to server");
                }
              });
        },
        ...
        error: function (jqXHR, textStatus, errorThrown) {
          ...
          $("header").css("background-color","#ff0000");
          alert("Prospect saved to device. Please upload later");
        }
    });
